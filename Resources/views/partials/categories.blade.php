@stack('pagination_start')
    @if ($categories->firstItem())
        <div class="col-xs-12 col-sm-5 d-flex align-categories-center">
            ...
            <span class="table-text d-none d-lg-block ml-2">
                ...
                {{ trans('pagination.showing', ['first' => $categories->firstItem(), 'last' => $categories->lastItem(), 'total' => $categories->total()]) }}
            </span>
        </div>

        <div class="col-xs-12 col-sm-7 pagination-xs">
            <nav class="float-right">
                {!! $categories->withPath(request()->url())->withQueryString()->links() !!}
            </nav>
        </div>
    @else
