...
class AmazonProductInfo extends AmazonProductsCore
{
     ...
     */
    protected function prepareMatching()
    {
        include $this->env;
    ...
    {
        if (!array_key_exists('SellerSKUList.SellerSKU.1', $this->options) && !array_key_exists('ASIN',
                $this->options)
        ) {
            $this->log('Product IDs must be set in order to look them up!', 'Warning');
    ...
    {
        include $this->env;
        if (isset($THROTTLE_TIME_CATEGORY)) {
            $this->throttleTime = $THROTTLE_TIME_CATEGORY;
        }
     ...
     * @return bool <b>FALSE</b> if something goes wrong
     */
    public function fetchLowestPrice()
    {
        if (!array_key_exists('SellerSKUList.SellerSKU.1', $this->options) && !array_key_exists('ASINList.ASIN.1',
                $this->options)
        ) {
