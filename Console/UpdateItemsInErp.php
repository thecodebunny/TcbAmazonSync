...
class UpdateItemsInErp extends Command
{
     ...
     * @var string
     */
    protected $signature = 'amazon:update-items-erp {country}';
    ...
    public function handle()
    {
        ...
        foreach ($companies as $company) {
            ...
            if ($settings && !empty($settings)) {
                ...
                $items = AmzItem::where('company_id', $company->id)->where('country', $country)->where('is_uploaded_uk', 1)->get();
                    ...
                    if (count($items)) {
                        foreach ($items as $item) {
                            ...
                            try {
                                ...
                                echo 'Error updating item in ERP in Country' . $country . ' For SKU ' . $item->sku . ' ERROR: ' . $e->getMessage() . '';
                            }
                            sleep(15);
                            continue;
                        }
    ...
    public function updateAmazonItem($companyId, $item, $dbItem)
    {
        ...
        if($dbItem->country == 'Uk') {
            $dbItem->is_uploaded_uk = true;
        }
        if($dbItem->country == 'De') {
            $dbItem->is_uploaded_de = true;
        }
        if($dbItem->country == 'Fr') {
            $dbItem->is_uploaded_fr = true;
        }
        if($dbItem->country == 'It') {
            $dbItem->is_uploaded_it = true;
        }
        if($dbItem->country == 'Es') {
            $dbItem->is_uploaded_es = true;
        }
        if($dbItem->country == 'Se') {
            $dbItem->is_uploaded_se = true;
        }
        if($dbItem->country == 'Nl') {
            $dbItem->is_uploaded_nl = true;
        }
        if($dbItem->country == 'Pl') {
            $dbItem->is_uploaded_pl = true;
        }
        if($dbItem->country == 'Us') {
            $dbItem->is_uploaded_us = true;
        }
        if($dbItem->country == 'Ca') {
            $dbItem->is_uploaded_ca = true;
        }
